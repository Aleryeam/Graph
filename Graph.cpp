...
//            else g[i][j] = INF;
    ...
    printf("接下来的m行：\n");
    printf("请输入两个整数a,b；浮点数w,表示a城市和b城市之间存在一条权值为w的边\n");
    //接下来 m 行，每行包含三个数 u,v,w，表示点 u 和点 v 之间存在一条权值为 w 的边。
    for (int i = 0; i < m; i++)
    {
        int a, b;
        double w;
        scanf_s("%d%d%lf", &a, &b, &w);
        g[a][b] = min(g[a][b], (int)w); //这里是有向图
